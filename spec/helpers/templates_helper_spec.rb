...
describe Templates::Helper do
  ...
  describe "template_part_field" do
    ...
    it "should determine the field name from the given index" do
      helper.should_receive(:hidden_field_tag).with('part[1][content]', '', :class => 'asset', :id => 'part_1_content')
    ...
    it "should put blank content in the field if the part doesn't already exist" do
      ...
      helper.should_receive(:text_field_tag).with('part[1][content]', '', :class => 'text', :style => "width: 500px", :id => 'part_1_content')
    ...
    if defined?(ConcurrentDraftExtension)
      it "should output the draft content and hidden live content if concurrent_draft is enabled" do
        helper.should_receive(:hidden_field_tag).with('part[1][content]', 'Just a test.', :id => 'part_1_content')
        helper.should_receive(:text_area_tag).with('part[1][draft_content]', '', :class => 'plain', :id => 'part_1_draft_content')
